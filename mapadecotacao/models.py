...
class CustomUser(AbstractUser):
    ...
    def __str__(self):
        # Mostra o nome completo (nome + sobrenome)
        return f'{self.name} {self.surname}'

    def get_profile_photo(self):
        # Retorna o caminho da foto de perfil, ou uma foto padrão se não houver
        if self.photo:
            return self.photo.url
        return '/static/default-profile-pic.png'  # Caminho de uma imagem padrão
...
class Orcamento(models.Model):
    usuario = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.CASCADE)  # Alterado para AUTH_USER_MODEL
...
class ComposicaoSinapi(models.Model):
    descricao = models.TextField()

class ComposicaoSicro3(models.Model):
    descricao = models.TextField()
